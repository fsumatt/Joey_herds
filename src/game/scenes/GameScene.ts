...
import { Joey } from '../entities/Joey';
...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.joey = new Joey(this, this.level.joeyStart);
    this.input.on('pointerdown', (p: Phaser.Input.Pointer) => this.joey.moveTo({ x: p.x, y: p.y }));
  ...
  update(_time: number, delta: number) {
    ...
    this.sheep.forEach((s) => s.updateSheep(delta, this.joey, this.level.obstacles, this.level.corral));
